          ...
          overflow-x-scroll scrollbar-none'>
              {
                  categories && categories.map(category => (
                    ...
                    whileTap={{scale:0.75}}
                    key={category.id} 
                    className={`group w-24 min-w-[94px] h-28 cursor-pointer rounded-lg ${filter === category.urlParamName? 'bg-cartNumBG': 'bg-white'}
                     drop-shadow-xl flex flex-col gap-3 justify-center hover:bg-cartNumBG items-center `}
                    onClick={()=>setFilter(category.urlParamName)} 
                     >
                        <div className={`w-10 h-10 shadow-lg ${filter === category.urlParamName? "bg-white": "bg-cartNumBG"} 
